    ...
    def __init__(self, latent_dim, input_dim, depth, heads, dim_head, dropout=0.):
        ...
        for _ in range(self.depth):
            self.layers.append(nn.ModuleList([
                PreNorm(latent_dim, Attention(latent_dim, input_dim, self.cross_heads, self.cross_dim_head, dropout = dropout), context_dim = input_dim),
...
class MLCL(BertPreTrainedModel):
    def __init__(self, config, multimodal_config, args = None):
        ...
        if self.num_labels == 1:
            ...
            self.loss = CrossEntropyLoss() # BCEWithLogitsLoss
    ...
    def get_network(self, self_type='l', layers=5):
        ...
        return TransformerEncoder(embed_dim=embed_dim,
                                  ...
                                  attn_mask= False)#self.attn_mask)# change the layers
    ...
    def forward(
        ...
        cross_vt = self.cross_vt(outputs_v1, outputs_t1)   #
        ...
        # return (infonce_ta + infonce_tv + infonce_av) / 12.0
        ...
        loss_task = self.loss(output.view(-1,2), label_ids.type(torch.cuda.LongTensor).view(-1))
        self.optimizer_all.zero_grad()

        loss_all = loss_task + contra_loss0 / 3 * 0.03 + contra_loss1 / 3 * 0.06 + contra_loss2 / 3 * 0.9 # 0.03, 0.06, 0.09

        loss_all.backward() #
        
        torch.nn.utils.clip_grad_value_([param for param in self.model_all_parameters if param.requires_grad], self.clip)
